...
- [x] 1. Add Express backend enforcement routes
  - [x] 1.1 Create `server/routes/profiles.js` with age-lock and role-validation endpoints
  ...
  - [x] 1.2 Create `server/routes/children.js` with child CRUD endpoints
  ...
  - [x] 1.3 Create `server/routes/parentalControls.js` with PIN-gated controls endpoints
  ...
  - [x] 1.4 Register new routes in `server/index.js`
    - Mount `profiles`, `children`, `parentalControls` routers after `express.json()`
    - _Requirements: 1.6, 4.3, 5.7_

- [x] 2. Checkpoint — verify backend routes respond correctly
  - Ensure all tests pass, ask the user if questions arise.

- [x] 3. Extend `src/lib/db.js` with missing data helpers
  - [x] 3.1 Add `getProgressReport(childId, period)` function
  ...
  - [x] 3.3 Add `deleteChildProfileCascade(childId, parentId)` that deletes profile + activity in a transaction
    - _Requirements: 4.8_
...
- [x] 4. Update `KidsModeContext` to expose `enableKidsMode` and enforce age-lock on disable
  - [x] 4.1 Export `enableKidsMode` from `KidsModeContext` so external callers (Profile.jsx, ChildSwitcherContext) can activate Kids Mode without going through `requestToggle`
    - Add `enableKidsMode` to the context value object
    - _Requirements: 2.1, 2.2_

  - [x] 4.2 Harden the `requestToggle('disable')` path: if `profile.age < 13`, show PIN modal and never deactivate without correct PIN
...
- [x] 5. Update `Profile.jsx` — age input with lock, role selector, auto Kids Mode
  - [x] 5.1 Harden the `AgeInput` block: add client-side validation rejecting values outside [1, 120]; show inline error; disable input when `is_age_locked = 1`
  ...
  - [x] 5.5 Update `RoleSelector` block: hide Parent/Teacher buttons when `profile.age < 13`; call `PATCH /api/profiles/:userId/role` on save
  ...
  - [x] 5.8 Wire auto Kids Mode: after a successful profile save where `age < 13`, call `enableKidsMode()` from `KidsModeContext`; persist Kids Mode state to profile via `db.upsertProfile`
...
- [x] 6. Checkpoint — verify profile save, age lock, role selector, and auto Kids Mode work end-to-end
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 7. Add `ManageChildrenSection` inside `Profile.jsx`
  - [x] 7.1 Render the "Manage Children" section only when `profile.role === 'Parent'`; hide for all other roles
  ...
  - [x] 7.3 Implement child creation form: display name (required), age, avatar picker; submit calls `POST /api/children/:parentId`; show error banner if max-6 is reached
  ...
  - [x] 7.6 Implement child deletion with PIN modal: clicking delete opens a PIN entry modal; on correct PIN calls `DELETE /api/children/:parentId/:childId`; refreshes child list
  ...
  - [x] 7.8 Auto-activate Kids Mode for newly created child profiles with age < 13 (set `kids_mode = true` on the child profile record via `db.upsertChildProfile`)
    - _Requirements: 4.5_

- [ ] 8. Create `src/components/ParentalControlsPanel.jsx`
  - [x] 8.1 Build the panel UI: AI feature toggles (`bible_character_chat`, `ai_prayer_companion`), Kids-routes-only toggle, daily time limit selector (0/15/30/45/60 min), PIN change field
    - _Requirements: 5.1, 5.3, 5.4, 5.5, 5.8_

  - [x] 8.2 On save, open a PIN confirmation modal; on correct PIN call `PUT /api/parental-controls/:parentId`; show inline error on wrong PIN or invalid limit
  ...
  - [x] 8.5 Wire `ParentalControlsContext` timer to `daily_limit` from the saved controls; ensure `TimeLimitOverlay` renders when `sessionTime >= daily_limit > 0`
    - _Requirements: 5.6_
...
- [ ] 9. Create `src/pages/ChildDashboard.jsx` at route `/profile/child/:childId`
  - [x] 9.1 Fetch child profile, activity stats (7-day and 30-day), streak, badges via `db.getChildActivityStats` and `db.getProgressReport`; render streak, badges earned, quiz scores, recent activity list
    - _Requirements: 4.6, 4.7_

  - [x] 9.2 Embed `ParentalControlsPanel` inside the dashboard, accessible only to the parent who owns the child profile
    - _Requirements: 5.1_

  - [x] 9.3 Add a "Family Win" banner: query `family_devotional_progress` for the current plan; if all assigned children completed the same day, show the celebration banner on next parent login
    - _Requirements: 6.5_

  - [x] 9.4 Add a missed-devotional reminder indicator: if any child has no completion record for 3+ consecutive days, show a gentle reminder badge on the dashboard
    - _Requirements: 6.6_

  - [x] 9.5 Register the route in `src/App.jsx` inside the `<PrivateRoute>` block: `<Route path="/profile/child/:childId" element={<SP c={ChildDashboard}/>}/>`
    - _Requirements: 4.6_

- [ ] 10. Create `src/components/ProgressReport.jsx`
  - [x] 10.1 Build the report component: period selector (7d / 30d / all), stat cards (streak, days read, badges, quizzes, accuracy), activity timeline list
  ...
  - [x] 10.3 Implement "Share Report" action: generate a static printable `<div>` containing only the child's display name and stats (no email, no user ID); trigger `window.print()` or copy-to-clipboard
  ...
  - [x] 10.5 Embed `ProgressReport` inside `ChildDashboard.jsx`
    - _Requirements: 7.2_

- [x] 11. Checkpoint — verify child dashboard, parental controls, and progress report work end-to-end
  - Ensure all tests pass, ask the user if questions arise.

- [ ] 12. Create `src/pages/ParentsTeachers.jsx` at route `/parents-teachers`
  - [x] 12.1 Build the page shell: hero section, category tabs (Lesson Plans, Devotionals, Activity Ideas, Teaching Resources), role-aware welcome message
  ...
  - [x] 12.2 Create `src/components/ResourceCard.jsx`: renders title, description, age-range tag, and action link/download button; accepts a `resource` prop
    - _Requirements: 3.3_
  ...
  - [x] 12.4 Implement role-based filtering: when `profile.role === 'Teacher'` surface teacher resources first; when `profile.role === 'Parent'` surface parent resources first; unauthenticated users see a limited preview set
  ...
  - [x] 12.6 Apply Kids Mode filter: when `kidsMode` is active, only render resources tagged `ageAppropriate: true`
    - _Requirements: 3.8_
  ...
  - [x] 12.8 Add unauthenticated gate: clicking a full resource when not signed in shows a sign-in prompt modal linking to `/auth`
    - _Requirements: 3.6, 3.7_

- [ ] 13. Create `src/components/FamilyDevotionalSection.jsx`
  - [x] 13.1 Build the assignment UI: plan selector (predefined list), start date picker, child multi-select (from `childProfiles`); submit calls `db.upsertFamilyPlan` then `db.updateFamilyProgress` for each selected child
  ...
  - [x] 13.3 Implement day-completion toggle: each child's day card has a "Mark Complete" button; calls `db.updateFamilyProgress(planId, childId, day, 'completed')`
    - _Requirements: 6.3_
  ...
  - [x] 13.5 Compute and display progress percentage: `floor(completedDays / totalDays * 100)`; render a progress bar per child
    - _Requirements: 6.4_
  ...
  - [x] 13.8 Embed `FamilyDevotionalSection` inside the `/parents-teachers` page under the "Devotionals" tab (visible to authenticated Parent users only)
    - _Requirements: 6.1_

- [ ] 14. Wire teacher aggregated progress view
  - [x] 14.1 When `profile.role === 'Teacher'`, render an "Aggregated Progress" section in the Content Hub that calls `GET /api/progress/:childId` for each linked child and displays anonymized averages (no names, only stats)
    - _Requirements: 7.4_
