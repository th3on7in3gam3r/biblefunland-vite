    ...
    const fetchAggregatedProgress = async () => {
      try {
        ...
        const avgDaysRead = Math.round(
          validResults.reduce((sum, r) => sum + (r.data.totalDaysRead || 0), 0) /
            validResults.length
        );
        ...
        const avgQuizzes = Math.round(
          validResults.reduce((sum, r) => sum + (r.data.quizzesCompleted || 0), 0) /
            validResults.length
        );
        const avgAccuracy = Math.round(
          validResults.reduce((sum, r) => sum + (r.data.quizAccuracy || 0), 0) / validResults.length
        );
        const totalQuizzes = validResults.reduce(
          (sum, r) => sum + (r.data.quizzesCompleted || 0),
          0
        );
    ...
    <div style={{ marginBottom: 32 }}>
      ...
      >
        ...
        <div style={{ fontSize: '.85rem', color: 'var(--ink2)', fontWeight: 500 }}>
          Anonymized averages across {studentCount} student{studentCount !== 1 ? 's' : ''} — no
