  ...
  return (
    <div style={{ fontFamily: 'Poppins,sans-serif' }}>
          ...
          {['7d', '30d', '90d'].map((p) => (
            <button
              ...
              style={{
                ...
                background: period === p ? 'var(--blue)' : 'var(--surface)',
                color: period === p ? 'white' : 'var(--ink3)',
                cursor: 'pointer',
              }}
            >
              {p}
      ...
      {!isConfigured && (
        ...
        >
          ⚠️ <strong>AdSense not configured.</strong> Add{' '}
          <code
            style={{
              background: 'var(--bg3)',
              padding: '1px 6px',
          ...
          file. Data below is <strong>simulated</strong>.
        </div>
      )}
        ...
        <div>
          ...
          >
            {[
              ...
              <div
                key={i}
                style={{ ...s.card, padding: 20, position: 'relative', overflow: 'hidden' }}
              >
                <div
                  style={{
                    position: 'absolute',
                ...
                <div style={{ fontSize: '.7rem', fontWeight: 700, color: 'var(--green)' }}>
            ...
            >
              {data.slice(-30).map((d, i) => (
                <div
                  ...
                  title={`${d.date}: $${d.earnings.toFixed(2)}`}
                >
                  <div
                    style={{
                      width: '100%',
            ...
            <div style={{ flex: 1 }}>
              ...
              >
                ...
                <span style={{ color: 'var(--green)' }}>
                  ${(parseFloat(totalEarnings) * (30 / parseInt(period))).toFixed(2)}
                </span>
              </div>
              <div
                style={{
              ...
              >
                At current RPM rates with 10% monthly traffic growth, you could hit{' '}
                <strong>$200-600/month</strong> within 6 months. Faith-based content commands
                premium CPMs ($6-15) from Christian advertisers.
              </div>
            </div>
            <a href="/premium" className="btn btn-green btn-sm">
              💎 Boost with Pro Tier
            </a>
          </div>
        </div>
      ...
      {tab === 'pages' && (
        <div style={s.card}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr>
                {['Page', 'Impressions', 'RPM', 'Earnings', 'Trend'].map((h) => (
                  <th key={h} style={s.th}>
                    {h}
                  </th>
                ))}
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontSize: '.72rem', color: 'var(--ink3)', fontWeight: 600 }}>
                  {unit.type} · CTR: {unit.ctr}
                </div>
              </div>
              {[
                ['Impressions', unit.impressions],
                ['RPM', unit.rpm],
                ['Earnings', unit.earnings],
              ].map(([l, v]) => (
                <div key={l} style={{ textAlign: 'center' }}>
                  ...
                  <div style={{ fontSize: '.65rem', color: 'var(--ink3)', fontWeight: 600 }}>
                    {l}
                  </div>
                </div>
              ))}
          ...
          {[
            {
              step: 1,
              status: 'done',
              title: 'Privacy Policy page created',
              desc: 'Available at /privacy — required by Google AdSense',
              icon: '📄',
            },
            {
              step: 2,
              status: 'done',
              title: 'Cookie Consent banner added',
              desc: 'GDPR & CCPA compliant — loads before any tracking',
              icon: '🍪',
            },
            {
              step: 3,
              status: 'done',
              title: 'Contact page at /contact',
              desc: 'Google requires a visible way to contact the site owner',
              icon: '📬',
            },
            {
              step: 4,
              ...
              desc: 'Added to /public — submit sitemap to Google Search Console',
              icon: '🗺️',
            },
            {
              step: 5,
              status: !isConfigured ? 'todo' : 'done',
              title: 'Add AdSense Publisher ID to .env',
              desc: 'VITE_ADSENSE_CLIENT=ca-pub-XXXXXXXXXX',
              icon: '🔑',
            },
            {
              step: 6,
              status: 'todo',
              title: 'Add AdSense script to index.html',
              desc: 'See AdUnit.jsx comments for the exact script tag',
              icon: '📝',
            },
            {
              step: 7,
              status: 'todo',
              title: 'Replace slot IDs in AdUnit.jsx',
              desc: 'Create ad units in AdSense dashboard, copy slot IDs',
              icon: '📣',
            },
            {
              step: 8,
              status: 'todo',
              title: 'Apply to Google AdSense',
              desc: 'Go to adsense.google.com → Add Site → biblefunland.com',
              icon: '✅',
            },
            {
              step: 9,
              status: 'todo',
              title: 'Add VITE_GA_MEASUREMENT_ID to .env',
              desc: 'Google Analytics 4 — free at analytics.google.com',
              icon: '📊',
            },
            {
              step: 10,
              ...
              desc: 'search.google.com/search-console → Sitemaps → Submit',
              icon: '🚀',
            },
          ].map((item) => (
            <div
              ...
              style={{
                ...
                borderRadius: 16,
                padding: '16px 20px',
                border: `1.5px solid ${item.status === 'done' ? 'rgba(16,185,129,.3)' : 'var(--border)'}`,
                background: item.status === 'done' ? 'var(--green-bg)' : 'var(--surface)',
              }}
            >
              <div
                style={{
                  ...
                  height: 32,
                  borderRadius: '50%',
                  background: item.status === 'done' ? 'var(--green)' : 'var(--bg3)',
                  display: 'flex',
                  alignItems: 'center',
                  justifyContent: 'center',
                  fontSize: item.status === 'done' ? '.9rem' : '.85rem',
                  flexShrink: 0,
                  fontFamily: "'Baloo 2',cursive",
                  fontWeight: 800,
                  color: item.status === 'done' ? 'white' : 'var(--ink3)',
                }}
              >
