...
const DEVOTIONAL_PLANS = [
  { id: 'prayer-journey', title: '30-Day Prayer Journey', days: 30, description: 'A guided journey through prayer with daily reflections' },
  { id: 'bible-stories', title: 'Bible Stories for Kids', days: 21, description: 'Classic Bible stories told in an engaging way' },
  { id: 'fruit-spirit', title: 'Fruit of the Spirit', days: 14, description: 'Explore the nine fruits of the Spirit' },
  { id: 'psalms-praise', title: 'Psalms of Praise', days: 30, description: 'Daily psalms for worship and gratitude' },
  { id: 'jesus-teachings', title: 'Jesus\' Teachings', days: 28, description: 'Core teachings of Jesus for families' },
  { id: 'creation-wonder', title: 'Creation Wonder', days: 7, description: 'Celebrating God\'s creation through daily devotions' }
]
  ...
  if (loading) {
    return (
      <div style={{ background: 'var(--surface)', borderRadius: 24, border: '1.5px solid var(--border)', padding: 24, boxShadow: 'var(--sh)', textAlign: 'center' }}>
        <div style={{ fontSize: '.9rem', color: 'var(--ink3)', fontWeight: 600 }}>Loading family devotionals...</div>
      </div>
    )
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      <div style={{ background: 'var(--surface)', borderRadius: 24, border: '1.5px solid var(--border)', padding: 24, boxShadow: 'var(--sh)' }}>
        <div style={{ fontSize: '.68rem', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>
          📖 Assign Family Devotional
        </div>
        <div style={{ fontSize: '.82rem', color: 'var(--ink2)', fontWeight: 500, marginBottom: 20 }}>
          Start a guided devotional journey with your children. Choose a plan, set a start date, and select which children will participate.
        </div>

        {/* Plan Selector */}
        <div style={{ marginBottom: 18 }}>
          <label style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--ink)', display: 'block', marginBottom: 8 }}>
        ...
        <div style={{ marginBottom: 18 }}>
          ...
          />
          <div style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 500, marginTop: 6 }}>
            {selectedPlan && DEVOTIONAL_PLANS.find(p => p.id === selectedPlan) && (
        ...
        {/* Child Multi-Select */}
        <div style={{ marginBottom: 18 }}>
          <label style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--ink)', display: 'block', marginBottom: 10 }}>
            Select Children ({selectedChildren.length} selected)
          </label>
          {childProfiles.length === 0 ? (
            <div style={{ fontSize: '.82rem', color: 'var(--ink3)', fontWeight: 500, padding: '12px 14px', background: 'var(--bg2)', borderRadius: 12, textAlign: 'center' }}>
              ...
              {childProfiles.map(child => (
                <div
                  ...
                  style={{
                    ...
                    padding: '12px 14px',
                    borderRadius: 12,
                    border: selectedChildren.includes(child.id) ? '2px solid var(--blue)' : '1.5px solid var(--border)',
                    background: selectedChildren.includes(child.id) ? 'var(--blue-bg)' : 'var(--bg2)',
                    cursor: 'pointer',
                    transition: 'all .2s'
                  }}
                >
                  <div style={{
                    ...
                    background: selectedChildren.includes(child.id) ? 'var(--blue)' : 'transparent',
                    display: 'flex',
                    alignItems: 'center',
                    ...
                    {selectedChildren.includes(child.id) && <span style={{ color: 'white', fontSize: '.8rem', fontWeight: 800 }}>✓</span>}
                  </div>
                  <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <button
            ...
            style={{
              ...
              borderRadius: 12,
              border: 'none',
              background: submitting || !selectedPlan || !startDate || selectedChildren.length === 0 ? 'var(--bg3)' : 'var(--blue)',
              color: submitting || !selectedPlan || !startDate || selectedChildren.length === 0 ? 'var(--ink3)' : 'white',
              fontFamily: 'Poppins,sans-serif',
              fontSize: '.82rem',
              fontWeight: 700,
              cursor: submitting || !selectedPlan || !startDate || selectedChildren.length === 0 ? 'default' : 'pointer',
              transition: 'all .2s'
            }}
          >
            {submitting ? '⏳ Assigning...' : '✅ Assign Devotional'}
          </button>
          {submitError && <div style={{ fontSize: '.76rem', color: 'var(--red)', fontWeight: 600 }}>⚠️ {submitError}</div>}
          {submitSuccess && <div style={{ fontSize: '.76rem', color: 'var(--green)', fontWeight: 600 }}>✓ Devotional assigned successfully!</div>}
        </div>
      </div>
      ...
      {familyPlans.length > 0 && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ fontSize: '.68rem', fontWeight: 800, color: 'var(--ink3)', textTransform: 'uppercase', letterSpacing: 1 }}>
            📊 Family Devotional Progress
          </div>

          {familyPlans.map(plan => {
            const planDef = DEVOTIONAL_PLANS.find(p => p.id === plan.title?.toLowerCase().replace(/\s+/g, '-'))
            const totalDays = plan.total_days || 30
            const planProgress = progressData[plan.id] || []

            return (
              <div key={plan.id} style={{ background: 'var(--surface)', borderRadius: 24, border: '1.5px solid var(--border)', padding: 20, boxShadow: 'var(--sh)' }}>
                <div style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--ink)', marginBottom: 16 }}>
                  {plan.title}
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  {selectedChildren.map(childId => {
                    ...
                    const completedDays = childProgress.filter(p => p.status === 'completed').length
                    const progressPercent = Math.floor((completedDays / totalDays) * 100)

                    return (
                      <div key={childId} style={{ background: 'var(--bg2)', borderRadius: 16, padding: 14 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                          <div style={{ fontSize: '.82rem', fontWeight: 700, color: 'var(--ink)' }}>
                            {child.display_name}
                          </div>
                          <div style={{ fontFamily: "'Baloo 2',cursive", fontSize: '.95rem', fontWeight: 800, color: 'var(--blue)' }}>
                            {progressPercent}%
                          </div>
                        </div>
                        ...
                        <div style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 500, marginTop: 10, textAlign: 'center' }}>
