...
function AdPlaceholder({ label, width, height, style = {} }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ fontSize: '.65rem', opacity: 0.7 }}>
        {CLIENT === 'ca-pub-XXXXXXXXXX' ? 'Set VITE_ADSENSE_CLIENT in .env' : CLIENT}
      </span>
    </div>
  );
}
...
export function AdBanner({ className = '' }) {
  ...
  if (import.meta.env.DEV)
    return (
      <AdPlaceholder
        label="Banner (728×90 / Responsive)"
        height={90}
        style={{ margin: '8px auto', maxWidth: 728 }}
      />
    );
  ...
  return (
    <div
      className={className}
      style={{ textAlign: 'center', overflow: 'hidden', padding: '8px 0' }}
    >
      <ins
        ref={ref}
...
export function AdSidebar({ className = '' }) {
  ...
  if (import.meta.env.DEV)
    return <AdPlaceholder label="Sidebar Rectangle (300×250)" width={300} height={250} />;
  if (!showAds) return null;
...
export function AdInContent({ className = '' }) {
  ...
  if (import.meta.env.DEV)
    return <AdPlaceholder label="In-Content Native Ad" height={120} style={{ margin: '24px 0' }} />;
  if (!showAds) return null;

  return (
    <div
      className={className}
      style={{ textAlign: 'center', overflow: 'hidden', margin: '24px 0' }}
    >
      <ins
        ref={ref}
        className="adsbygoogle"
...
export function AdStickyFooter() {
  ...
  if (import.meta.env.DEV) {
    return (
      ...
      >
        <AdPlaceholder
          label="Sticky Footer Ad (728×90)"
          height={60}
          style={{ margin: '0 auto', maxWidth: 728 }}
        />
        <button
...
export function AdAfterGame({ score, className = '' }) {
  ...
  return (
    <div className={className} style={{ margin: '20px 0' }}>
      ...
      >
        Advertisement —{' '}
        <a href="/premium" style={{ color: 'var(--blue)', fontWeight: 700 }}>
