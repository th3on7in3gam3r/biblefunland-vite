  ...
  <>
    ...
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 'auto' }}>
      ...
      {item.tags?.split(',').map(t => (
        <span key={t} style={{ fontSize: '0.65rem', color: 'var(--ink3)', fontWeight: 600 }}>#{t.trim()}</span>
      ))}
    </div>
  </>
);
...
export const MentorCard = ({ item }) => (
  <>
    ...
    <div style={{ marginTop: 'auto', display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.7rem', color: item.still_connected ? 'var(--green)' : 'var(--ink3)', fontWeight: 700 }}>
...
export const VerseCard = ({ item }) => (
  <>
    ...
    <p className={styles.cardText}>
      ...
      {item.how_it_helped && (
        <><br/><br/><strong>Comfort:</strong> {item.how_it_helped}</>
      )}
    </p>
    <div className={styles.cardDate} style={{ marginTop: 'auto' }}>🗓️ {fmtDate(item.season_date)}</div>
  </>
);

export const PrayerCard = ({ item }) => {
  const days = Math.floor((new Date(item.answer_date) - new Date(item.prayer_date)) / (1000 * 60 * 60 * 24));
  return (
    <>
      ...
      <div className={styles.verseDisplay} style={{ borderLeftColor: 'var(--blue)', color: 'var(--blue)' }}>
