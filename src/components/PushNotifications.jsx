...
export default function PushNotifications() {
    ...
    if (p === 'granted') {
      ...
      setTimeout(() => {
        sendNotification(
          ...
          '"The Lord is my shepherd; I shall not want." — Psalm 23:1\n\nYou\'ll receive your daily verse at ' +
            time
        );
      }, 1000);
    }
  ...
  function sendTest() {
    ...
    sendNotification(
      '☀️ BibleFunLand — Daily Verse',
      VERSES[day % VERSES.length] + ' — Tap to read more'
    );
  }
  ...
  const stateColor =
    { granted: 'var(--green)', denied: 'var(--red)', default: 'var(--orange)' }[permission] ||
    'var(--orange)';
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 16 }}>
          ...
          <div>
            ...
            <div style={{ fontSize: '.72rem', fontWeight: 600, color: stateColor }}>
              ●{' '}
              {permission === 'granted'
                ? registered
                  ? 'Active'
        ...
        >
          Receive a beautiful scripture verse every morning at your chosen time — even when your
          browser is closed.
        </p>
        {permission !== 'denied' && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 14 }}>
            ...
            <input
              ...
              onChange={(e) => {
                ...
                localStorage.setItem('bfl_notif_time', e.target.value);
              }}
              style={{
                padding: '7px 11px',
                borderRadius: 9,
        ...
        <div style={{ display: 'flex', gap: 9, flexWrap: 'wrap' }}>
          ...
          {permission === 'denied' && (
            <p style={{ fontSize: '.78rem', color: 'var(--red)', fontWeight: 500 }}>
              Notifications are blocked. Go to your browser settings → Site Settings → Notifications
              to re-enable.
            </p>
          )}
        </div>
      ...
      >
        ...
        {showReminderForm && (
          <div
            style={{ background: 'var(--bg2)', padding: 16, borderRadius: 12, marginBottom: 16 }}
          >
            <div style={{ display: 'grid', gap: 12 }}>
              <div>
                <label style={{ fontSize: '.75rem', fontWeight: 700, color: 'var(--ink3)' }}>
                  Title
                </label>
                <input
                  type="text"
                  ...
                  {['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'].map((day) => (
                    <button
                      ...
                      style={{
                        padding: '6px',
                        borderRadius: 6,
                        background: newReminder.days.includes(day)
                          ? 'var(--blue)'
                          : 'var(--border)',
                        color: newReminder.days.includes(day) ? 'white' : 'var(--ink3)',
                        border: 'none',
                        cursor: 'pointer',
                        fontSize: '.7rem',
                        fontWeight: 600,
            ...
            {reminders.map((reminder) => (
              ...
              >
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '.75rem', color: 'var(--ink3)', marginTop: 4 }}>
