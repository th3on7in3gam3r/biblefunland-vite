...
export default function DavidGoliath() {
  ...
  const [highScore, setHighScore] = useState(() =>
    parseInt(localStorage.getItem('dg_high') || '0')
  );
      ...
      scene: {
        ...
        create() {
          ...
          scene.hpText = scene.add
            .text(GAME_CONFIG.width / 2, 14, getHpString(goliathHpLocal), {
              fontSize: '18px',
              fill: '#EF4444',
          ...
          scene.scoreText = scene.add
            .text(GAME_CONFIG.width - 14, 14, `Score: ${scoreLocal}`, {
              fontSize: '16px',
              fill: '#1F2937',
              fontFamily: 'Poppins,sans-serif',
            })
              ...
              callback: () => {
                ...
                if (dist < 50) {
                  ...
                  if (goliathHpLocal <= 0) {
                    ...
                    const win = scene.add
                      .text(GAME_CONFIG.width / 2, GAME_CONFIG.height / 2 - 40, '🏆 GIANT SLAIN!', {
                        fontSize: '40px',
                        fill: '#F59E0B',
                        fontFamily: 'Baloo 2,cursive',
                        fontStyle: 'bold',
                    ...
                    scene.add
                      .text(
                        GAME_CONFIG.width / 2,
                        GAME_CONFIG.height / 2 + 14,
                        `Score: ${scoreLocal}`,
                        { fontSize: '22px', fill: '#1F2937', fontFamily: 'Poppins,sans-serif' }
                      )
                      .setOrigin(0.5);
      ...
      >
        <h1
          style={{
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#FCD34D,#FB923C,#EC4899)',
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '36px 20px' }}>
        ...
        {gameState === 'menu' && (
          <div>
            <div style={{ textAlign: 'center', marginBottom: 28 }}>
              ...
              {highScore > 0 && (
                <div style={{ fontSize: '.85rem', color: 'var(--ink3)', fontWeight: 600 }}>
            ...
            >
              {GAME_CONFIG.levels.map((l, i) => (
                <div
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = l.color + '22';
                  }}
                  onMouseLeave={(e) => {
          ...
          <div>
            ...
            >
              ...
              >
                <span
                  style={{
                    background: lvlCfg.color + '22',
                    color: lvlCfg.color,
                    padding: '3px 10px',
                    borderRadius: 100,
                    fontSize: '.78rem',
                    border: `1px solid ${lvlCfg.color}`,
                  }}
                >
                  {lvlCfg.label}
                </span>
        ...
        {gameState === 'win' && (
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            <div
              style={{ fontSize: '5rem', marginBottom: 16, animation: 'bounce 1s ease infinite' }}
            >
              🏆
            </div>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              {level < GAME_CONFIG.levels.length - 1 && (
                <button className="btn btn-blue" onClick={() => startGame(level + 1)}>
                  Next Level →
                </button>
              )}
              <button className="btn btn-outline" onClick={() => startGame(level)}>
                Play Again
              </button>
              <button className="btn btn-outline" onClick={cleanup}>
                Menu
              </button>
            </div>
          </div>
        ...
        {gameState === 'lose' && (
          <div style={{ textAlign: 'center', padding: '40px 20px' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center', flexWrap: 'wrap' }}>
              <button className="btn btn-orange" onClick={() => startGame(level)}>
                Try Again
              </button>
              <button className="btn btn-outline" onClick={cleanup}>
                Choose Level
