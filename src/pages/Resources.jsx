...
function ProductCard({ product }) {
  ...
  return (
    <div className={styles.card}>
      {product.badge && (
        <div className={styles.cardBadge} style={{ background: product.badgeColor || '#6366F1' }}>
          {product.badge}
        </div>
      )}
      ...
      <div className={styles.imageArea}>
        {!imgError ? (
          ...
          <div className={styles.emojiFallback}>{product.emoji || '📖'}</div>
        )}
    ...
    <div className={styles.resources}>
      ...
      <section className={styles.hero}>
        <div className={styles.heroContent}>
          <div className={styles.badge}>🛍️ Kingdom Affiliate Store</div>
          <h1 className={styles.title}>Resources for the Journey</h1>
          <p className={styles.subtitle}>
            A curated collection of Bibles, devotionals, and tools to help your family grow in
            faith. Every purchase supports our mission to make the Bible fun for everyone.
          </p>
      ...
      <div className={styles.disclosure}>
        💛 Transparency Note: As an Amazon Associate, BibleFunLand earns from qualifying purchases
        ...
        {staffPicks.length > 0 && (
          <div style={{ marginBottom: 56 }}>
            ...
            <div className={styles.grid}>
              {staffPicks.map((p) => (
                <ProductCard key={`staff-${p.id}`} product={p} />
              ))}
            </div>
