...
export default function ChristianFinance() {
  ...
  const breakdown = [
    { label: 'Tithe (First Fruits)', pct: 10, amount: income * 0.1, color: 'var(--blue)' },
    { label: 'Savings & Investing', pct: 10, amount: income * 0.1, color: '#F59E0B' },
  ...
  const handleAddDebt = () => {
    ...
    setDebts([
      ...debts,
      { id: Date.now(), name: newDebtName, total: Number(newDebtTotal), paid: 0 },
    ]);
    setNewDebtName('');
    setNewDebtTotal('');
  };
  ...
  const handleAddGoal = () => {
    ...
    setGoals([
      ...goals,
      { id: Date.now(), title: generosityTitle, target: Number(generosityTarget), current: 0 },
    ]);
    setGenerosityTitle('');
    setGenerosityTarget('');
  };
            ...
            >
              ...
              >
                ...
                <div style={{ marginBottom: 16 }}>
                  ...
                  >
                    <span style={{ fontSize: '1.2rem', color: 'var(--ink)', fontWeight: 700 }}>
                      $
                    </span>
                    <input
                      type="number"
              ...
              >
                  ...
                  {breakdown.map((item, i) => (
                    <div key={i}>
                      ...
                      >
                        ...
                        <span style={{ fontSize: '.95rem', fontWeight: 700, color: 'var(--ink)' }}>
                ...
                <div
                  style={{ marginTop: 24, padding: 16, background: 'var(--bg2)', borderRadius: 12 }}
                >
                  <p
                    style={{
                      margin: 0,
                  ...
                  >
                    *This breakdown aligns with the classic Christian stewardship model: give God
                    your first fruits (10%), save/invest for the future (10%), be generous (5%), and
                    live joyfully on the remaining 75%.
                  </p>
                </div>
              </div>
            </div>
                        ...
                        <button
                          ...
                          onClick={() => {
                            setDebts(
                              debts.map((d) =>
                                d.id === debt.id
                                  ? { ...d, paid: Math.min(d.total, d.paid + 500) }
                                  : d
                              )
                            );
                          }}
        ...
        {tab === 'generosity' && (
          <div style={{ animation: 'fadeUp .4s ease both' }}>
                ...
                return (
                  ...
                  >
                    ...
                    >
                      ...
                      <span style={{ fontSize: '.9rem', color: 'var(--ink3)', fontWeight: 600 }}>
                        / ${goal.target} goal
                      </span>
                    </div>
                    <div
                    ...
                    <button
                      ...
                      onClick={() => {
                        setGoals(
                          goals.map((g) =>
                            g.id === goal.id
                              ? { ...g, current: Math.min(g.target, g.current + 50) }
                              : g
                          )
                        );
                      }}
            ...
            >
              ...
              >
                Set New Generosity Goal
              </div>
              <input
                type="text"
                placeholder="e.g. Sponsor a child"
                value={generosityTitle}
                onChange={(e) => setGenerosityTitle(e.target.value)}
                style={{
                  flex: 1,
                  minWidth: 200,
                  padding: 12,
              ...
              <div style={{ position: 'relative' }}>
                ...
                <input
                  type="number"
                  placeholder="Target"
                  value={generosityTarget}
                  onChange={(e) => setGenerosityTarget(e.target.value)}
                  style={{
                    width: 120,
