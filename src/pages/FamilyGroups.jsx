...
function getGroups() {
  try {
    return JSON.parse(localStorage.getItem('bfl_groups') || '[]');
  } catch {
    return [];
  }
}
function saveGroups(g) {
  localStorage.setItem('bfl_groups', JSON.stringify(g));
}
...
export default function FamilyGroups() {
  ...
  function joinGroup() {
    const demo = {
      ...
      prayers: ['Pray for our family vacation!', "Thanksgiving for Sarah's healing"],
      createdAt: new Date().toISOString(),
    };
      ...
      <div style={{ maxWidth: 800, margin: '0 auto', padding: '36px 20px' }}>
        {!user && (
          ...
          >
            ...
            to create or join a group that syncs across devices.
          </div>
        )}
        {msg && (
          <div
                  ...
                  >
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: '.73rem', color: 'var(--ink3)', fontWeight: 500 }}>
                        {grp.members.length} members · Code: <strong>{grp.code}</strong>
                      </div>
                    </div>
                    <div style={{ fontSize: '.78rem', fontWeight: 700, color: 'var(--blue)' }}>
                      Open →
                    </div>
                  </div>
                ))}
        ...
        {view === 'create' && (
          ...
          >
            ...
            >
              💡 After creating, you'll get a <strong>6-character invite code</strong> to share with
              family members.
            </div>
            <div style={{ display: 'flex', gap: 12 }}>
              <button className="btn btn-blue" onClick={createGroup} disabled={!groupName.trim()}>
                Create Group →
              </button>
        ...
        {view === 'join' && (
          ...
          >
            ...
            />
            <div style={{ display: 'flex', gap: 12 }}>
              <button className="btn btn-green" onClick={joinGroup} disabled={joinCode.length < 4}>
                Join Group →
              </button>
              <button className="btn btn-outline" onClick={() => setView('list')}>
                Cancel
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 20 }}>
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <div style={{ fontSize: '.75rem', color: 'var(--ink3)', fontWeight: 500 }}>
                    Invite Code:{' '}
                    <strong style={{ color: 'var(--blue)', letterSpacing: 2 }}>{g.code}</strong> ·
                    Share with family!
                  </div>
                </div>
              </div>
                  ...
                  >
                    <div
                      style={{
                        ...
                        fontSize: '1.3rem',
                        fontWeight: 800,
                        color: ['#FFD700', '#C0C0C0', '#CD7F32', 'var(--ink3)'][i] || 'var(--ink3)',
                        width: 28,
                        textAlign: 'center',
                      }}
                    >
