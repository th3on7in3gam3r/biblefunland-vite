...
function getReadData() {
  try {
    return JSON.parse(localStorage.getItem('bfl_read_books') || '{}');
  } catch {
    return {};
  }
}
...
function getStreakData() {
  try {
    return JSON.parse(localStorage.getItem('bfl_reading_log') || '[]');
  } catch {
    return [];
  }
}
...
export default function ReadingStats() {
  ...
  // Stats
  const chaptersRead = Object.entries(readBooks).reduce(
    (n, [book, chs]) => n + (Array.isArray(chs) ? chs.length : 0),
    0
  );
  const booksComplete = Object.entries(readBooks).filter(
    ([book, chs]) => Array.isArray(chs) && chs.length >= (CHAPTERS[book] || 1)
  ).length;
  ...
  const currentStreak = (() => {
    ...
    for (let i = 0; i < 365; i++) {
      ...
      d.setDate(d.getDate() - i);
      if (readData[d.toISOString().split('T')[0]]) s++;
      else break;
    }
    return s;
  })();
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      <div
        style={{
          background: 'linear-gradient(135deg,#0F0F1A,#0A1A14)',
      ...
      >
        <h1
          style={{
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#6EE7B7,#60A5FA,#FCD34D)',
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          Your Bible reading journey, visualized. Every chapter counts.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 960, margin: '0 auto', padding: '28px 20px' }}>
        ...
        >
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 10 }}>
            ...
            >
              {chaptersRead} / {TOTAL_CHAPTERS} chapters
            </div>
          </div>
          <div
            style={{ height: 16, borderRadius: 100, background: 'var(--bg3)', overflow: 'hidden' }}
          >
            <div
              style={{
                height: '100%',
                ...
                width: `${pctComplete}%`,
                transition: 'width .5s',
                boxShadow: '0 0 12px rgba(52,211,153,.3)',
              }}
            />
          </div>
          <div style={{ fontSize: '.72rem', color: 'var(--ink3)', fontWeight: 500, marginTop: 8 }}>
            {1189 - chaptersRead} chapters remaining to complete the entire Bible
          </div>
        </div>
          ...
          {[
            ...
            <button
              ...
              style={{
                ...
                padding: '9px 18px',
                borderRadius: 12,
                border: `1.5px solid ${tab === id ? 'var(--green)' : 'var(--border)'}`,
                background: tab === id ? 'var(--green-bg)' : 'var(--surface)',
                color: tab === id ? 'var(--green)' : 'var(--ink3)',
                cursor: 'pointer',
                transition: 'all .2s',
              }}
            >
        ...
        {tab === 'heatmap' && (
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
              {dates365.map((d) => {
                ...
                return (
                  <div
                    key={d}
                    title={`${d}: ${count} session${count !== 1 ? 's' : ''}`}
                    style={{
                      width: 14,
                      height: 14,
                      borderRadius: 3,
            ...
            <div style={{ display: 'flex', gap: 6, alignItems: 'center', marginTop: 12 }}>
              <span style={{ fontSize: '.68rem', color: 'var(--ink3)', fontWeight: 500 }}>
                Less
              </span>
              {['var(--bg3)', '#34D399', '#059669', '#065F46', '#064E3B'].map((c, i) => (
        ...
        {tab === 'overview' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            >
              ...
              {[
                ['Old Testament', BOOKS_OT, 'var(--yellow)'],
                ['New Testament', BOOKS_NT, 'var(--blue)'],
              ].map(([label, books, color]) => {
                    ...
                    >
                      <div
                        style={{
                          ...
                          background:
                            dayCount[i] === Math.max(...dayCount) ? 'var(--green)' : 'var(--bg3)',
                          minHeight: 4,
                        }}
                      />
                      <div style={{ fontSize: '.62rem', fontWeight: 700, color: 'var(--ink3)' }}>
                        {day}
                      </div>
                    </div>
                  );
        ...
        {tab === 'books' && (
          <div>
            {[
              ['📜 Old Testament', BOOKS_OT],
              ['✝️ New Testament', BOOKS_NT],
            ].map(([label, books]) => (
                    ...
                    return (
                      ...
                      >
                        ...
                        >
                          ...
                          <span
                            style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 600 }}
                          >
                            {readChs}/{total}
                          </span>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3 }}>
                          {Array.from({ length: total }, (_, i) => i + 1).map((ch) => (
                            <div
                              key={ch}
                              onClick={() => toggleChapter(book, ch)}
                              title={`Chapter ${ch}`}
                              style={{
                                width: 14,
                                height: 14,
                                borderRadius: 3,
                                background: (readBooks[book] || []).includes(ch)
                                  ? 'var(--green)'
                                  : 'var(--bg3)',
                                cursor: 'pointer',
                                transition: 'all .15s',
