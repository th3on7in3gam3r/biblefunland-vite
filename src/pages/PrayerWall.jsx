...
export default function PrayerWall() {
  ...
  function pray(id) {
    setPrayers((ps) =>
      ps.map((p) =>
        p.id === id ? { ...p, count: p.prayed ? p.count - 1 : p.count + 1, prayed: !p.prayed } : p
      )
    );
  }
  function submit() {
    ...
    const n = {
      id: Date.now(),
      name: form.anon ? 'Anonymous' : form.name || 'Anonymous',
      category: form.category,
      text: form.text,
      time: 'Just now',
      bg: CBGS[form.category] || 'var(--green-bg)',
      emoji: '🙏',
      count: 0,
      prayed: false,
    };
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.6)', fontSize: '.9rem', fontWeight: 500 }}>
          Share your heart. Pray for others. Faith grows together.
        </p>
      </div>
      <div style={{ maxWidth: 1060, margin: '0 auto', padding: '44px 24px' }}>
        ...
        >
          ...
          >
            Share what's on your heart and let the community lift you up in prayer.
          </div>
          {submitted && (
            <div
              style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
                ...
                <div
                  style={{
                    ...
                    padding: '3px 9px',
                    borderRadius: 100,
                    background: CBGS[p.category] || 'var(--green-bg)',
                    color: CCOLORS[p.category] || 'var(--green)',
                  }}
                >
                  {p.category}
                </div>
              ...
              >
                <button
                  ...
                  style={{
                    ...
                    padding: '7px 14px',
                    borderRadius: 9,
                    border: `1.5px solid ${p.prayed ? 'var(--green)' : 'var(--border)'}`,
                    background: p.prayed ? 'var(--green-bg)' : 'var(--surface)',
                    color: p.prayed ? 'var(--green)' : 'var(--ink2)',
                    cursor: 'pointer',
                    transition: 'all .2s',
                  }}
                >
