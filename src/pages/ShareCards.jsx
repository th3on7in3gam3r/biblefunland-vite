...
export default function ShareCards(){
  ...
  const[verse,setVerse]=useState('"For God so loved the world that he gave his one and only Son, that whoever believes in him shall not perish but have eternal life."')
  ...
  function renderCard(){
    ...
    const ctx=canvas.getContext('2d');const W=540,H=540
    const theme=THEMES[themeIdx]
    const grad=ctx.createLinearGradient(0,0,W,H);grad.addColorStop(0,theme.bg1);grad.addColorStop(1,theme.bg2)
    ctx.fillStyle=grad;ctx.fillRect(0,0,W,H)
    ctx.beginPath();ctx.arc(W-60,60,120,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,.04)';ctx.fill()
    ctx.beginPath();ctx.arc(60,H-60,90,0,Math.PI*2);ctx.fillStyle='rgba(255,255,255,.04)';ctx.fill()
    ctx.font='bold 140px serif';ctx.fillStyle='rgba(255,255,255,.04)';ctx.textAlign='center';ctx.fillText('✝',W/2,H/2+50)
    ctx.font='bold 18px sans-serif';ctx.fillStyle=theme.accent;ctx.textAlign='left';ctx.fillText('✝ BibleFunLand',32,42)
    ctx.fillStyle=theme.text;ctx.textAlign='center'
    const words=verse.replace(/^[""]/,'').replace(/[""]/,'').split(' ')
    const maxW=W-100;let line='';let y=200
    const fontSize=words.length>20?18:words.length>12?21:24
    ctx.font=`600 ${fontSize}px sans-serif`
    words.forEach((w,i)=>{const test=line+w+' ';if(ctx.measureText(test).width>maxW&&i>0){ctx.fillText(line.trim(),W/2,y);line=w+' ';y+=fontSize*1.4}else line=test})
    if(line.trim())ctx.fillText(line.trim(),W/2,y)
    ctx.font='700 19px sans-serif';ctx.fillStyle=theme.accent;ctx.fillText('— '+ref,W/2,y+44)
    ctx.fillStyle='rgba(255,255,255,.15)';ctx.fillRect(W/2-80,H-54,160,1)
    ctx.font='500 13px sans-serif';ctx.fillStyle='rgba(255,255,255,.35)';ctx.fillText('biblefunland.com',W/2,H-30)
  }
  ...
  return(
    <div style={{background:'var(--bg)',minHeight:'100vh',fontFamily:'Poppins,sans-serif'}}>
      <div style={{background:'linear-gradient(135deg,#0F0F1A,#1C1B3A)',padding:'60px 36px 44px',textAlign:'center'}}>
        <h1 style={{fontFamily:"'Baloo 2',cursive",fontSize:'clamp(2rem,4.5vw,3.4rem)',fontWeight:800,letterSpacing:-1,background:'linear-gradient(90deg,#F472B6,#C084FC,#60A5FA)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text',marginBottom:8}}>Share Cards</h1>
        <p style={{color:'rgba(255,255,255,.5)',fontSize:'.9rem',fontWeight:500,maxWidth:440,margin:'0 auto'}}>Create beautiful verse graphics to share on Instagram, Facebook, WhatsApp, and more.</p>
      </div>
      <div style={{maxWidth:960,margin:'0 auto',padding:'44px 24px'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:28,alignItems:'start'}}>
          <div style={{background:'var(--surface)',borderRadius:24,border:'1.5px solid var(--border)',boxShadow:'var(--sh)',padding:28}}>
            <div style={{fontSize:'.72rem',fontWeight:700,color:'var(--ink3)',letterSpacing:'.5px',textTransform:'uppercase',marginBottom:8}}>Verse</div>
            <textarea className="textarea-field" rows={3} value={verse} onChange={e=>{setVerse(e.target.value)}} style={{marginBottom:10}}/>
            <input className="input-field" value={ref} onChange={e=>setRef(e.target.value)} style={{marginBottom:22}}/>
            <div style={{fontSize:'.72rem',fontWeight:700,color:'var(--ink3)',letterSpacing:'.5px',textTransform:'uppercase',marginBottom:10}}>Background Theme</div>
            <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:8,marginBottom:22}}>
              {THEMES.map((t,i)=>(
                <div key={i} onClick={()=>setThemeIdx(i)} style={{height:40,borderRadius:9,cursor:'pointer',background:`linear-gradient(135deg,${t.bg1},${t.bg2})`,border:`2px solid ${i===themeIdx?'var(--blue)':'transparent'}`,boxShadow:i===themeIdx?'0 0 0 3px rgba(59,130,246,.2)':'none',transition:'all .2s'}}/>
              ))}
            </div>
            <button className="btn btn-blue btn-full" onClick={renderCard} style={{justifyContent:'center'}}>🎨 Update Card</button>
          </div>
