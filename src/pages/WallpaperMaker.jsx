...
const VERSES = [
  ...
  {
    ref: 'Rom 8:38',
    text: 'Neither death nor life, neither angels nor demons, neither the present nor the future, nor any powers — nothing will be able to separate us from the love of God.',
  },
  {
    ref: 'Isa 40:31',
...
const BACKGROUNDS = [
  ...
  },
  {
    ...
    draw: (ctx, W, H) => {
      ...
      for (let i = 0; i < 8; i++) {
        ctx.fillRect(0, H * 0.5 + i * 30, W, 20);
      }
    },
  },
  {
  ...
  },
  {
    ...
    draw: (ctx, W, H) => {
      ...
      g.addColorStop(0.5, '#92400e');
      g.addColorStop(1, '#1c1917');
      ctx.fillStyle = g;
      ctx.fillRect(0, 0, W, H);
      ctx.fillStyle = 'rgba(252,211,77,.08)';
      const rg = ctx.createRadialGradient(W / 2, H * 0.25, 0, W / 2, H * 0.25, 300);
      rg.addColorStop(0, 'rgba(252,211,77,.25)');
...
export default function WallpaperMaker() {
  ...
  useEffect(() => {
    render();
  }, [verseIdx, customVerse, customRef, bgId, fontId, textColor]);
  ...
  function render() {
    ...
    ctx.globalAlpha = 0.2;
    ctx.beginPath();
    ctx.moveTo(W * 0.25, H / 2 + totalH / 2 + 18);
    ctx.lineTo(W * 0.75, H / 2 + totalH / 2 + 18);
    ctx.stroke();
    ctx.globalAlpha = 1;
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        <h1
          style={{
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#C084FC,#F472B6,#FCD34D)',
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          Create a phone lock screen wallpaper with your favorite Bible verse. Download at full
          resolution — free.
        </p>
      </div>
      ...
      >
        ...
        <div>
          ...
          >
              ...
              {VERSES.map((v, i) => (
                <button
                  ...
                  style={{
                    ...
                    padding: '5px 11px',
                    borderRadius: 100,
                    border: `1.5px solid ${verseIdx === i && !customVerse ? 'var(--violet)' : 'var(--border)'}`,
                    background:
                      verseIdx === i && !customVerse ? 'var(--violet-bg)' : 'var(--surface)',
                    color: verseIdx === i && !customVerse ? 'var(--violet)' : 'var(--ink2)',
                    cursor: 'pointer',
                  }}
                >
                  {v.ref}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 120px', gap: 8 }}>
              <input
                ...
                placeholder="Or type your own verse text..."
                value={customVerse}
              ...
              {BACKGROUNDS.map((b) => (
                <button
                  ...
                  style={{
                    padding: '9px 6px',
                    borderRadius: 10,
                    border: `2px solid ${bgId === b.id ? 'var(--violet)' : 'var(--border)'}`,
                    background: bgId === b.id ? 'var(--violet-bg)' : 'var(--bg2)',
                    color: bgId === b.id ? 'var(--violet)' : 'var(--ink3)',
          ...
          >
                ...
                {FONTS.map((f) => (
                  <button
                    ...
                    style={{
                      padding: '9px 12px',
                      borderRadius: 10,
                      border: `2px solid ${fontId === f.id ? 'var(--blue)' : 'var(--border)'}`,
                      background: fontId === f.id ? 'var(--blue-bg)' : 'var(--bg2)',
                      color: fontId === f.id ? 'var(--blue)' : 'var(--ink3)',
            ...
            <div>
              ...
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                {['#FFFFFF', '#FCD34D', '#A5B4FC', '#6EE7B7', '#F9A8D4', '#1F2937'].map((c) => (
                  <button
                    key={c}
                    ...
                    style={{
                      ...
                      borderRadius: '50%',
                      background: c,
                      border: `3px solid ${textColor === c ? 'var(--blue)' : 'transparent'}`,
                      cursor: 'pointer',
                      transition: 'all .2s',
                      boxShadow:
                        textColor === c ? '0 0 0 2px var(--bg),0 0 0 4px var(--blue)' : 'none',
                    }}
                  />
                ))}
              </div>
          ...
          >
            Portrait format · Phone lock screen ready · Free to use
          </p>
        </div>
        ...
        <div style={{ position: 'sticky', top: 80, textAlign: 'center' }}>
          ...
          <div
            style={{
              borderRadius: 20,
              overflow: 'hidden',
              boxShadow: '0 20px 60px rgba(0,0,0,.35), 0 0 0 2px rgba(255,255,255,.08)',
          ...
          >
            <canvas
              ...
              width={W}
              height={H}
              style={{ display: 'block', width: '100%', maxWidth: 270, height: 'auto' }}
            />
          </div>
          <p style={{ fontSize: '.68rem', color: 'var(--ink3)', marginTop: 8 }}>
            540×960 preview · Downloads at 1080×1920
          </p>
        </div>
      </div>
    </div>
