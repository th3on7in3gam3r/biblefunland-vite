...
export default function BibleSearch() {
  ...
  const displayBooks =
    testament === 'All' ? [...BOOKS_OT, ...BOOKS_NT] : testament === 'OT' ? BOOKS_OT : BOOKS_NT;
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      <div
        style={{
          background: 'linear-gradient(135deg,#0F0F1A,#0A1A0F)',
      ...
      >
        <h1
          style={{
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#6EE7B7,#60A5FA,#A5B4FC)',
      ...
      <div style={{ maxWidth: 900, margin: '0 auto', padding: '32px 20px' }}>
        ...
        >
          <div style={{ display: 'flex', gap: 10, marginBottom: 14 }}>
            <input
              ...
              placeholder='Search the Bible: "peace", "do not fear", "John 3:16"...'
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              onKeyDown={(e) => e.key === 'Enter' && search()}
              style={{ flex: 1, fontSize: '1rem', borderRadius: 12 }}
              autoFocus
            />
            <button
            ...
            {['All', 'OT', 'NT'].map((t) => (
              <button
                ...
                style={{
                  ...
                  borderRadius: 100,
                  cursor: 'pointer',
                  border: `1.5px solid ${testament === t ? 'var(--blue)' : 'var(--border)'}`,
                  background: testament === t ? 'var(--blue)' : 'var(--surface)',
                  color: testament === t ? 'white' : 'var(--ink3)',
                  transition: 'all .2s',
                }}
              >
        ...
        {!apiKey && (
          ...
          >
            ⚠️ <strong>API key not set.</strong> Showing demo results. Add{' '}
            <code>VITE_BIBLE_API_KEY</code> to .env for full Bible search via scripture.api.bible
            (free).
          </div>
        )}
        ...
        {results &&
          ...
          (results.length === 0 ? (
            ...
            >
              ...
              <p style={{ fontSize: '.84rem', color: 'var(--ink3)' }}>
                Try a different word or phrase. Use simple keywords for best results.
              </p>
            </div>
          ) : (
              ...
              {results.map((verse, i) => (
                ...
                >
                  ...
                  >
                    ...
                    >
                      {verse.reference || verse.bookname}
                    </div>
                    <button
                      onClick={() =>
                        navigator.clipboard.writeText(
                          `${verse.text || verse.verse} — ${verse.reference || verse.bookname}`
                        )
                      }
                      style={{
                        background: 'none',
        ...
        {!results && !loading && (
          <div style={{ textAlign: 'center', paddingTop: 40 }}>
            <div style={{ fontSize: '5rem', opacity: 0.15, marginBottom: 16 }}>📖</div>
            <div
              style={{
                fontFamily: "'Baloo 2',cursive",
                fontSize: '1.3rem',
            ...
            <p
              style={{ fontSize: '.88rem', color: 'var(--ink3)', maxWidth: 400, margin: '0 auto' }}
            >
              Enter a word, phrase, or reference above. Results highlight your search term in
              context.
            </p>
          </div>
        )}
