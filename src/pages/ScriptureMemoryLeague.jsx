...
export default function ScriptureMemoryLeague() {
  ...
  function nextVerse() {
    ...
    if (remaining.length === 0 || round >= ROUNDS) {
      finishGame();
      return;
    }
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          Fill in the missing word. Score by speed. Climb the leaderboard. Weekly reset every
          Monday.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 760, margin: '0 auto', padding: '32px 20px' }}>
        {phase === 'menu' && (
          <div>
            ...
            >
              {[
                ['🏆', `${hiScore}pts`, 'Your Best Score', 'var(--yellow)'],
                ['📅', 'Weekly', 'Resets Monday', 'var(--blue)'],
                ['⚡', ROUNDS, 'Rounds per game', 'var(--green)'],
              ].map(([e, v, l, c], i) => (
                <div
                  key={i}
                  style={{
              ...
              {LEADERBOARD.map((p) => (
                <div
                  ...
                  style={{
                    ...
                    gap: 12,
                    padding: '11px 0',
                    borderBottom: p.rank < 5 ? '1px solid var(--border)' : 'none',
                  }}
                >
                  <div style={{ fontSize: '1.2rem', width: 28, textAlign: 'center' }}>
                    {p.badge}
                  </div>
                  <div
                    style={{
            ...
            <button
              ...
              style={{
                ...
                borderRadius: 16,
                border: 'none',
                background: 'linear-gradient(135deg,#1C1645,#2D1B69)',
                color: 'white',
          ...
          <div>
            ...
            >
              ...
              >
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          borderRadius: 100,
                          background: timeLeft <= 3 ? '#EF4444' : '#10B981',
                ...
                <div
                  style={{ fontSize: '1.05rem', color: 'white', lineHeight: 1.75, fontWeight: 500 }}
                >
                  {current.display.split(' ').map((word, i) =>
                    word === '___' ? (
                      <span
                        key={i}
                        style={{
                ...
                {feedback === null ? (
                  <div style={{ display: 'flex', gap: 10 }}>
                    <input
                      ...
                      autoComplete="off"
                    />
                    <button className="btn btn-violet" onClick={submit}>
                      Submit →
                    </button>
                  </div>
                ) : (
                  <div
                    style={{ textAlign: 'center', padding: '12px 0', animation: 'popIn .3s ease' }}
                  >
                    {feedback === 'correct' && (
                      <div
                        style={{
                      ...
                      >
                        ✅ Correct! +{Math.max(10, timeLeft * 15) + streak * 5} pts
                      </div>
                    )}
                    {feedback === 'wrong' && (
                      <div
                      ...
                      >
                        ❌ The word was:{' '}
                        <span style={{ color: 'var(--orange)' }}>{current.answer}</span>
                      </div>
                    )}
                    {feedback === 'timeout' && (
                      <div
                      ...
                      >
                        ...
                        <span style={{ color: 'white' }}>{current.answer}</span>
                      </div>
                    )}
                  </div>
                )}
        ...
        {phase === 'done' && (
          ...
          >
            ...
            >
              {[
                ['⚡', 'Max Streak', `${maxStreak} 🔥`, 'var(--orange)'],
                ['✅', 'Rounds', `${round}/${ROUNDS}`, 'var(--blue)'],
                ['🏆', 'Best', `${Math.max(score, hiScore)}pts`, 'var(--yellow)'],
              ].map(([e, l, v, c], i) => (
                <div
                  key={i}
                  style={{ background: 'var(--bg2)', borderRadius: 14, padding: '12px 8px' }}
                >
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button className="btn btn-violet" onClick={startGame}>
                🔄 Play Again
              </button>
