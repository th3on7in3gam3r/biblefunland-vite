...
export default function SermonIllustrations() {
  ...
  const [saved, setSaved] = useState(() =>
    JSON.parse(localStorage.getItem('bfl_illustrations') || '[]')
  );

  async function generate() {
    const subject = mode === 'passage' ? passage : theme;
    if (!subject.trim()) {
      setError('Enter a passage or theme.');
      return;
    }
    setError('');
    setLoading(true);
    ...
    const audienceNote = {
      general: 'Mixed congregation',
      youth: 'Youth group (teens)',
      kids: 'Children (ages 6-12)',
      mens: "Men's group",
  ...
  function saveIllustration() {
    const entry = {
      id: Date.now(),
      subject: mode === 'passage' ? passage : theme,
    ...
    return text.split('\n').map((line, i) => {
      ...
      if (/^📖/.test(line))
        return (
          <div
            key={i}
            style={{ fontSize: '.8rem', fontWeight: 700, color: 'var(--green)', marginTop: 6 }}
          >
            {line}
          </div>
        );
      if (/^Preacher/.test(line))
        return (
          <div
            key={i}
            style={{
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          AI generates 4 vivid, memorable illustrations for any passage or theme. Fresh every time.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '32px 20px' }}>
        ...
        >
          ...
          {mode === 'passage' ? (
            <div style={{ marginBottom: 16 }}>
                ...
                {PASSAGE_SUGGESTIONS.map((p) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 11px',
                      borderRadius: 100,
                      border: `1.5px solid ${passage === p ? 'var(--orange)' : 'var(--border)'}`,
                      background: passage === p ? 'var(--orange-bg)' : 'var(--surface)',
                      color: passage === p ? 'var(--orange)' : 'var(--ink2)',
                      cursor: 'pointer',
                    }}
                  >
                    {p}
              ...
              <input
                ...
                placeholder='Or type your own: "Luke 24:1-12", "The Prodigal Son"...'
                value={passage}
            ...
            <div style={{ marginBottom: 16 }}>
                ...
                {THEMES.map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 11px',
                      borderRadius: 100,
                      border: `1.5px solid ${theme === t ? 'var(--orange)' : 'var(--border)'}`,
                      background: theme === t ? 'var(--orange-bg)' : 'var(--surface)',
                      color: theme === t ? 'var(--orange)' : 'var(--ink2)',
                      cursor: 'pointer',
                    }}
                  >
                    {t}
              ...
              <input
                className="input-field"
                placeholder='Or type your own: "Trusting God in uncertainty"...'
                value={theme}
              ...
              {[
                ...
                <button
                  ...
                  style={{
                    ...
                    padding: '7px 14px',
                    borderRadius: 10,
                    border: `1.5px solid ${audience === v ? 'var(--orange)' : 'var(--border)'}`,
                    background: audience === v ? 'var(--orange-bg)' : 'var(--surface)',
                    color: audience === v ? 'var(--orange)' : 'var(--ink3)',
                    cursor: 'pointer',
                    transition: 'all .2s',
                  }}
                >
              ...
              {TYPES.map((t) => (
                <button
                  ...
                  style={{
                    ...
                    padding: '5px 11px',
                    borderRadius: 100,
                    border: `1.5px solid ${types.includes(t) ? 'var(--blue)' : 'var(--border)'}`,
                    background: types.includes(t) ? 'var(--blue-bg)' : 'var(--surface)',
                    color: types.includes(t) ? 'var(--blue)' : 'var(--ink2)',
                    cursor: 'pointer',
                    transition: 'all .2s',
                  }}
                >
          ...
          >
            <span style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 600 }}>
              ✨ Powered by secure AI proxy. Premium illustration engine active.
            </span>
            <button
        ...
        {result && !loading && (
          ...
          >
            ...
            >
              ...
              >
                🎤 Illustrations: {mode === 'passage' ? passage : theme} ·{' '}
                {
                  {
                    general: 'General',
                    youth: 'Youth',
        ...
        {saved.length > 0 && (
          <div style={{ marginTop: 24 }}>
            ...
            >
              💾 Saved Illustrations ({saved.length})
            </div>
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    ...
                    <button
                      onClick={() => {
                        setSaved((sv) => {
                          const n = sv.filter((x) => x.id !== s.id);
                          localStorage.setItem('bfl_illustrations', JSON.stringify(n));
                          return n;
                        });
                      }}
                      style={{
