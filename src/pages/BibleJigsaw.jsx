...
export default function BibleJigsaw() {
  ...
  return (
    <div
      style={{
        background: 'linear-gradient(180deg,#EFF6FF 0%,#FDF4FF 100%)',
        minHeight: '100vh',
    ...
    >
      <div
        style={{
          background: `linear-gradient(135deg,${puzzle.color},${puzzle.color}bb)`,
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.8)', fontSize: '.82rem', fontWeight: 600 }}>
          Tap the pieces to add them to the picture!
        </p>
      </div>
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: '20px 14px' }}>
          ...
          {PUZZLES.map((p, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: 100,
                cursor: 'pointer',
                border: `2px solid ${puzzleIdx === i ? p.color : '#E5E7EB'}`,
                background: puzzleIdx === i ? p.color + '18' : 'white',
                color: puzzleIdx === i ? p.color : '#6B7280',
                transition: 'all .2s',
                display: 'flex',
                alignItems: 'center',
                gap: 5,
        ...
        <div
          style={{
            ...
            borderRadius: 22,
            padding: 12,
            boxShadow: `0 8px 32px ${puzzle.color}22`,
            marginBottom: 14,
            border: `2px solid ${puzzle.color}33`,
          }}
        >
          <div
            style={{
          ...
          <svg
            viewBox="0 0 400 300"
            style={{ width: '100%', borderRadius: 14, background: '#F9FAFB', display: 'block' }}
          >
            {puzzle.pieces.map((p) => (
              ...
              {puzzle.pieces.map((p) => (
                <button
                  ...
                  style={{
                    padding: '10px 8px',
                    borderRadius: 14,
                    border: `2px solid ${placed.has(p.id) ? '#D1FAE5' : puzzle.color + '66'}`,
                    background: placed.has(p.id) ? '#ECFDF5' : 'white',
                    cursor: placed.has(p.id) ? 'default' : 'pointer',
                    opacity: placed.has(p.id) ? 0.5 : 1,
                    transition: 'all .2s',
                    fontFamily: 'Poppins,sans-serif',
                    fontSize: '.7rem',
                    fontWeight: 700,
                    color: placed.has(p.id) ? '#059669' : '#374151',
                    textAlign: 'center',
                    lineHeight: 1.4,
                  }}
                  onMouseEnter={(e) => {
                    if (!placed.has(p.id)) e.currentTarget.style.background = puzzle.color + '14';
                  }}
                  onMouseLeave={(e) => {
                    if (!placed.has(p.id)) e.currentTarget.style.background = 'white';
                  }}
                >
        ...
        {showFact && (
          <div
            style={{
              ...
              border: `2px solid ${puzzle.color}44`,
          ...
          >
            ...
            <div
              style={{ fontSize: '.86rem', color: '#374151', lineHeight: 1.7, marginBottom: 18 }}
            >
              🌟 {puzzle.fact}
            </div>
