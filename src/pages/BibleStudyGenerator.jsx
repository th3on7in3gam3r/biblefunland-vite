...
export default function BibleStudyGenerator() {
  ...
  async function generate() {
    ...
    if (!subject.trim()) {
      setError('Please enter a topic or select a book.');
      return;
    }
    setError('');
    setLoading(true);
  ...
  function renderResult(text) {
    return text.split('\n').map((line, i) => {
      ...
      if (
        /^Scripture:|^Key Insight:|^Memory Verse:|^Application Challenge:|^Facilitator Notes/.test(
          line.trim()
        )
      )
        return (
      ...
      if (/^Discussion Questions:/.test(line.trim()))
        return (
          <div
            key={i}
            style={{
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        >
          📖 Replace $200 Curriculum · Free for Every Small Group
        </div>
        <h1
          style={{
            fontFamily: "'Baloo 2',cursive",
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#A5B4FC,#FCD34D)',
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          AI builds a complete multi-week small group curriculum with discussion questions,
          application challenges, and facilitator notes.
        </p>
      </div>
        ...
        >
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  padding: '11px',
                  borderRadius: 12,
                  border: `2px solid ${mode === id ? 'var(--violet)' : 'var(--border)'}`,
                  background: mode === id ? 'var(--violet-bg)' : 'var(--surface)',
                  color: mode === id ? 'var(--violet)' : 'var(--ink3)',
          ...
          {mode === 'topic' ? (
            <div style={{ marginBottom: 16 }}>
                ...
                {TOPICS.map((t) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 11px',
                      borderRadius: 100,
                      border: `1.5px solid ${topic === t ? 'var(--violet)' : 'var(--border)'}`,
                      background: topic === t ? 'var(--violet-bg)' : 'var(--surface)',
                      color: topic === t ? 'var(--violet)' : 'var(--ink2)',
                      cursor: 'pointer',
                    }}
                  >
                    {t}
              ...
              <input
                ...
                placeholder='Or type your own: "Anxiety", "Parenting", "The Cross"...'
                value={topic}
                ...
                {BOOKS.map((b) => (
                  <button
                    ...
                    style={{
                      ...
                      padding: '5px 11px',
                      borderRadius: 100,
                      border: `1.5px solid ${book === b ? 'var(--violet)' : 'var(--border)'}`,
                      background: book === b ? 'var(--violet-bg)' : 'var(--surface)',
                      color: book === b ? 'var(--violet)' : 'var(--ink2)',
                      cursor: 'pointer',
                    }}
                  >
                    {b}
              ...
              {LENGTHS.map((l) => (
                <button
                  ...
                  style={{
                    ...
                    padding: '7px 14px',
                    borderRadius: 10,
                    border: `1.5px solid ${length === l ? 'var(--blue)' : 'var(--border)'}`,
                    background: length === l ? 'var(--blue-bg)' : 'var(--surface)',
                    color: length === l ? 'var(--blue)' : 'var(--ink3)',
                    cursor: 'pointer',
                    transition: 'all .2s',
                  }}
                >
          ...
          >
            <span style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 600 }}>
              ✨ Powered by secure AI proxy. Advanced study engine active.
            </span>
            <button
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p style={{ fontSize: '.78rem', color: 'var(--ink3)', marginTop: 5 }}>
              Building all {length} weeks with discussion questions
            </p>
          </div>
        )}
