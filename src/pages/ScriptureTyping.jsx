...
export default function ScriptureTyping() {
  ...
  const [bestWpm, setBestWpm] = useState(() =>
    parseInt(localStorage.getItem('bfl_typing_best') || '0')
  );
  ...
  const handleInput = useCallback(
    (e) => {
      ...
      if (val === verse.text) {
        ...
        if (finalWpm > bestWpm) {
          setBestWpm(finalWpm);
          localStorage.setItem('bfl_typing_best', finalWpm);
        }
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          Type the verse exactly as shown. Speed and accuracy tracked. The best way to memorize.
        </p>
      ...
      <div style={{ maxWidth: 820, margin: '0 auto', padding: '28px 20px' }}>
          ...
          {VERSES.map((v, i) => (
            <button
              ...
              style={{
                ...
                borderRadius: 100,
                cursor: 'pointer',
                border: `1.5px solid ${verseIdx === i ? 'var(--green)' : 'var(--border)'}`,
                background: verseIdx === i ? 'var(--green-bg)' : 'var(--surface)',
                color: verseIdx === i ? 'var(--green)' : 'var(--ink3)',
                transition: 'all .2s',
              }}
            >
              {v.ref}
        ...
        >
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              <div
                style={{
                  fontSize: '.72rem',
                  fontWeight: 700,
                  color: accuracy >= 95 ? '#34D399' : accuracy >= 80 ? '#F59E0B' : '#EF4444',
                }}
              >
                🎯 {accuracy}%
              </div>
          ...
          {/* Rendered verse */}
          <div
            style={{ lineHeight: 1.9, marginBottom: 24, userSelect: 'none' }}
          ...
          <button
            ...
            onMouseEnter={(e) => {
              ...
              e.currentTarget.style.color = 'rgba(96,165,250,.6)';
            }}
            onMouseLeave={(e) => {
          ...
          >
            ...
            <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginBottom: 20 }}>
              {[
                ['⚡', wpm, 'WPM'],
                ['🎯', accuracy + '%', 'Accuracy'],
                ['⏱', Math.round(elapsed / 1000) + 's', 'Time'],
              ].map(([e, v, l]) => (
                <div
                  key={l}
                  style={{
            ...
            <div style={{ display: 'flex', gap: 10, justifyContent: 'center' }}>
              <button className="btn btn-green" onClick={nextVerse}>
                Next Verse →
              </button>
        ...
        >
          {[
            ['⌨️', 'Type exactly as shown — punctuation included'],
            ['🎯', 'Aim for 95%+ accuracy for a badge'],
            ['🔄', 'Green = correct · Red = needs fixing'],
            ['📖', 'Research shows typing = 3× better memorization than reading'],
          ].map(([e, t]) => (
            <div
              key={t}
              style={{ display: 'flex', gap: 8, alignItems: 'flex-start', flex: '1 1 200px' }}
            >
