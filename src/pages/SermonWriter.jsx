...
const LENGTHS = [
  { id: 'short', label: 'Short (15–20 min)', desc: '~1,500 words' },
  { id: 'medium', label: 'Standard (30–35 min)', desc: '~2,800 words' },
  { id: 'long', label: 'Full (45–50 min)', desc: '~4,000 words' },
];
...
export default function SermonWriter() {
  ...
  async function generate() {
    ...
    if (!subject.trim()) {
      setError('Enter a passage or topic.');
      return;
    }
    setError('');
    setLoading(true);
  ...
  function saveSermon() {
    const entry = {
      id: Date.now(),
      subject: mode === 'passage' ? passage : topic,
      tone,
    ...
    setSaved(n);
    localStorage.setItem('bfl_sermons', JSON.stringify(n));
  }
  ...
  function renderResult(text) {
    return text.split('\n').map((line, i) => {
      ...
      if (/^(SERMON TITLE|PASSAGE|THEME|ESTIMATED TIME):/.test(line))
        return (
          <div
            key={i}
            style={{
      ...
      if (/^POINT [123]:|^TRANSITION:/.test(line))
        return (
          <div
            key={i}
            style={{
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      <div
        style={{
          background: 'linear-gradient(135deg,#0F0F1A,#1C0A00)',
      ...
      >
        ...
        >
          ✍️ Full Manuscript · Ready to Preach Sunday
        </div>
        <h1
          style={{
            fontFamily: "'Baloo 2',cursive",
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          Complete sermon manuscripts — title, three points, illustrations, altar call, preacher's
          notes. Ready to preach.
        </p>
      </div>
      <div style={{ maxWidth: 880, margin: '0 auto', padding: '32px 20px' }}>
        ...
        >
            ...
            {[
              ...
              <button
                ...
                style={{
                  ...
                  padding: 11,
                  borderRadius: 12,
                  border: `2px solid ${mode === id ? 'var(--orange)' : 'var(--border)'}`,
                  background: mode === id ? 'var(--orange-bg)' : 'var(--surface)',
                  color: mode === id ? 'var(--orange)' : 'var(--ink3)',
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            >
              {mode === 'passage' ? 'Scripture Passage' : 'Sermon Topic or Theme'}
            </label>
            <input
              className="input-field"
              placeholder={
                mode === 'passage'
                  ? 'e.g. Romans 8:28-39, The Prodigal Son, Psalm 23...'
                  : 'e.g. Overcoming Fear, The Power of Prayer, Identity in Christ...'
              }
              value={mode === 'passage' ? passage : topic}
              onChange={(e) =>
                mode === 'passage' ? setPassage(e.target.value) : setTopic(e.target.value)
              }
                ...
                {TONES.map((t) => (
                  <button
                    ...
                    style={{
                      padding: '7px 10px',
                      borderRadius: 9,
                      border: `1.5px solid ${tone === t ? 'var(--orange)' : 'var(--border)'}`,
                      background: tone === t ? 'var(--orange-bg)' : 'var(--surface)',
                      color: tone === t ? 'var(--orange)' : 'var(--ink2)',
                      fontFamily: 'Poppins,sans-serif',
                      fontSize: '.72rem',
                      fontWeight: 700,
                      cursor: 'pointer',
                ...
                {AUDIENCES.map((a) => (
                  <button
                    ...
                    style={{
                      padding: '7px 10px',
                      borderRadius: 9,
                      border: `1.5px solid ${audience === a ? 'var(--blue)' : 'var(--border)'}`,
                      background: audience === a ? 'var(--blue-bg)' : 'var(--surface)',
                      color: audience === a ? 'var(--blue)' : 'var(--ink2)',
                      fontFamily: 'Poppins,sans-serif',
                      fontSize: '.72rem',
                      fontWeight: 700,
                      cursor: 'pointer',
              ...
              {LENGTHS.map((l) => (
                <div
                  ...
                  style={{
                    padding: '12px',
                    borderRadius: 12,
                    border: `2px solid ${length === l.id ? 'var(--green)' : 'var(--border)'}`,
                    background: length === l.id ? 'var(--green-bg)' : 'var(--surface)',
                    marginBottom: 8,
                    cursor: 'pointer',
                    transition: 'all .2s',
                  }}
                >
                  <div
                    style={{
                      fontSize: '.78rem',
                      fontWeight: 700,
                      color: length === l.id ? 'var(--green)' : 'var(--ink)',
                    }}
                  >
                    {l.label}
                  </div>
                  <div style={{ fontSize: '.66rem', color: 'var(--ink3)', fontWeight: 500 }}>
                    {l.desc}
                  </div>
                </div>
              ))}
          ...
          >
            <span style={{ fontSize: '.7rem', color: 'var(--ink3)', fontWeight: 600 }}>
              ✨ Powered by secure AI proxy. No local API keys required.
            </span>
            <button
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '48px 0' }}>
            ...
            <p style={{ color: 'var(--ink2)', fontWeight: 600 }}>
              Writing your sermon manuscript...
            </p>
            <p style={{ fontSize: '.78rem', color: 'var(--ink3)', marginTop: 5 }}>
              This takes ~20–30 seconds for a full manuscript
            </p>
          </div>
        )}
        ...
        {result && !loading && (
          ...
          >
            ...
            >
              ...
              >
                ✍️ {mode === 'passage' ? passage : topic} · {tone} · {audience}
              </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {saved.map((s) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    <button onClick={() => setResult(s.text)} className="btn btn-outline btn-sm">
                      View
                    </button>
                    <button
                      onClick={() => {
                        const n = saved.filter((x) => x.id !== s.id);
                        setSaved(n);
                        localStorage.setItem('bfl_sermons', JSON.stringify(n));
                      }}
                      style={{
