...
const STYLES = [
  {
    ...
    label: '🎤 Scripture Rap',
    icon: '🎤',
    desc: 'Hard-hitting bars rooted in the Word',
  },
  {
    id: 'gospel',
    label: '🎵 Gospel Song',
    icon: '🎵',
    desc: 'Soulful gospel with a call & response chorus',
  },
  {
    id: 'worship',
    label: '🙌 Worship Song',
    icon: '🙌',
    desc: 'Contemporary worship with bridge & chorus',
  },
  {
    id: 'spoken',
    label: '📣 Spoken Word',
    icon: '📣',
    desc: 'Poetic spoken word like spoken-word artists',
  },
  {
    id: 'kids',
    label: '🌈 Kids Song',
    icon: '🌈',
    desc: 'Fun, catchy song kids can memorize and sing',
  },
  {
    id: 'country',
    label: '🤠 Country Gospel',
    icon: '🤠',
    desc: 'Southern country gospel with storytelling',
  },
];
...
export default function BibleRapGenerator() {
  ...
  async function generate() {
    if (!verse.trim() && !topic.trim()) {
      setError('Enter a Bible verse or topic first!');
      return;
    }
    setError('');
    setLoading(true);
  ...
  function renderResult(text) {
    ...
    return lines.map((line, i) => {
      const isSection = /^\[(Verse|Chorus|Bridge|Pre-Chorus|Intro|Outro|Hook)\s*\d*\]/i.test(
        line.trim()
      );
      if (isSection)
        return (
      ...
      return (
        <div
          key={i}
          style={{ fontSize: '.92rem', color: 'var(--ink)', lineHeight: 1.75, fontWeight: 500 }}
        >
          {line}
        </div>
      );
  ...
  return (
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      <div
        style={{
          background: 'linear-gradient(135deg,#0F0F1A,#1A0533,#0F0F1A)',
      ...
      >
        ...
        <h1
          style={{
            ...
            fontWeight: 800,
            background: 'linear-gradient(90deg,#C084FC,#F472B6,#FCD34D,#60A5FA)',
        ...
        >
          Type any Bible verse or topic — AI writes a full scripture rap, worship song, spoken word,
          or kids song in seconds.
        </p>
      </div>
      ...
      <div style={{ maxWidth: 860, margin: '0 auto', padding: '40px 20px' }}>
          ...
          {STYLES.map((s) => (
            <div
              ...
              style={{
                ...
                padding: '16px 14px',
                cursor: 'pointer',
                border: `2px solid ${style === s.id ? 'var(--violet)' : 'var(--border)'}`,
                background: style === s.id ? 'var(--violet-bg)' : 'var(--surface)',
                textAlign: 'center',
                transition: 'all .2s',
              }}
            >
              ...
              <div
                style={{
                  ...
                  fontSize: '.9rem',
                  fontWeight: 800,
                  color: style === s.id ? 'var(--violet)' : 'var(--ink)',
                  marginBottom: 3,
                }}
              >
                {s.label}
        ...
        >
            ...
            {QUICK_VERSES.map((v) => (
              <button
                ...
                style={{
                  ...
                  padding: '4px 11px',
                  borderRadius: 100,
                  border: `1.5px solid ${verse === v ? 'var(--violet)' : 'var(--border)'}`,
                  background: verse === v ? 'var(--violet-bg)' : 'var(--surface)',
                  color: verse === v ? 'var(--violet)' : 'var(--ink2)',
                  cursor: 'pointer',
                }}
              >
                {v}
          ...
          <input
            ...
            placeholder='Bible verse e.g. "Philippians 4:13"...'
            value={verse}
            onChange={(e) => {
          ...
          <input
            className="input-field"
            placeholder="Topic e.g. 'overcoming fear', 'God's love', 'forgiveness'..."
            value={topic}
            onChange={(e) => {
        ...
        {loading && (
          <div style={{ textAlign: 'center', padding: '40px 0' }}>
            ...
            <p
              style={{ fontSize: '.9rem', color: 'var(--ink2)', fontWeight: 600, marginBottom: 6 }}
            >
              Writing your {selectedStyle?.label}...
            </p>
            <p style={{ fontSize: '.78rem', color: 'var(--ink3)', fontWeight: 500 }}>
              The Spirit is moving 🕊️
            </p>
          </div>
        )}
        ...
        {result && !loading && (
          ...
          >
            ...
            <div style={{ padding: '28px 32px' }}>{renderResult(result)}</div>
            <div
              style={{
                padding: '14px 28px',
                borderTop: '1px solid var(--border)',
