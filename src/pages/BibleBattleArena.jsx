...
const QUESTIONS = [
  ...
  },
  {
    ...
    options: ['David', 'Solomon', 'Saul', 'Gideon'],
    correct: 2,
  },
  {
  ...
  {
    q: 'Who built the Temple in Jerusalem?',
    options: ['David', 'Solomon', 'Rehoboam', 'Zerubbabel'],
    correct: 1,
  },
  {
    q: 'How many fruits of the Spirit are listed in Galatians 5?',
    options: ['7', '8', '9', '10'],
    correct: 2,
  },
  {
    q: 'What is the shortest verse in the Bible?',
    options: ['John 11:35', 'John 3:16', 'Ps 117:2', 'Rev 1:1'],
    correct: 0,
  },
  {
    q: 'Which prophet was swallowed by a great fish?',
    options: ['Elijah', 'Elisha', 'Jonah', 'Nahum'],
    correct: 2,
  },
  { q: 'How many disciples did Jesus choose?', options: ['10', '11', '12', '13'], correct: 2 },
  {
    q: 'What does the name "Jesus" mean?',
    options: ['God saves', 'God loves', 'God reigns', 'God heals'],
    correct: 0,
  },
  {
  ...
  {
    q: 'In which book does the burning bush appear?',
    options: ['Genesis', 'Leviticus', 'Numbers', 'Exodus'],
    correct: 3,
  },
  { q: 'How many plagues did God send on Egypt?', options: ['7', '8', '10', '12'], correct: 2 },
  {
    q: "What was Paul's name before his conversion?",
    options: ['Barnabas', 'Saul', 'Simon', 'Stephen'],
    correct: 1,
  },
];
...
export default function BibleBattleArena() {
  ...
  function resolveRound(hostAnswer) {
    ...
    if (hostCorrect && !opponentCorrect) {
      newScore.me += 1;
      result = 'host';
    } else if (opponentCorrect && !hostCorrect) {
      newScore.them += 1;
      result = 'opponent';
    } else if (hostCorrect && opponentCorrect) {
      newScore.me += 0.5;
      newScore.them += 0.5;
      result = 'both';
    }
    ...
    channelRef.current?.send({
      type: 'broadcast',
      event: 'result',
      payload: { score: newScore, result, round: nextRound },
    });
    setRound(nextRound);
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', fontFamily: 'Poppins,sans-serif' }}>
      ...
      >
        ...
        <p style={{ color: 'rgba(255,255,255,.45)', fontSize: '.88rem', fontWeight: 500 }}>
          1v1 live Bible trivia. Share your room code. First to answer correctly wins the point. 10
          rounds.
        </p>
      </div>
        ...
        {phase === 'menu' && (
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
            ...
            >
              ...
              >
                Create a room, share the code with a friend
              </p>
              <button
          ...
          >
            {isHost ? (
              <>
                ...
                >
                  Share this code with a friend. The battle starts when they join.
                </p>
                {waitingForOpponent ? (
                  <div
                    style={{
                  ...
                  >
                    ✅ Opponent joined! Starting in 1.5s...
                  </div>
                )}
              </>
            ) : (
                    ...
                    >
                      <div
                        style={{
                          height: '100%',
                          borderRadius: 100,
                          background: timeLeft <= 3 ? '#EF4444' : '#F97316',
              ...
              >
                {question.options.map((opt, i) => {
                  ...
                  if (show && isCorrect) {
                    ...
                    border = 'var(--green)';
                    color = 'var(--green)';
                  } else if (show && isMyAnswer && !isCorrect) {
                    bg = 'var(--red-bg)';
                    border = 'var(--red)';
                    color = 'var(--red)';
                  }
                  return (
                    <button
                      ...
                      style={{
                        ...
                        fontSize: '.88rem',
                        fontWeight: isMyAnswer || isCorrect ? 700 : 500,
                        transition: 'all .25s',
                      }}
                    >
              ...
              {roundResult && (
                <div
                  style={{
                    ...
                    fontSize: '1.1rem',
                    color:
                      (roundResult === 'host' && isHost) || (roundResult === 'opponent' && !isHost)
                        ? 'var(--green)'
                        : roundResult === 'both'
                          ? 'var(--yellow)'
                          : 'var(--red)',
                ...
                >
                  {roundResult === 'both'
                    ...
                    : (roundResult === 'host' && isHost) || (roundResult === 'opponent' && !isHost)
                      ? '✅ You got the point!'
                      : (roundResult === 'host' && !isHost) ||
                          (roundResult === 'opponent' && isHost)
                        ? '❌ Opponent got it'
                        : '⏰ Nobody scored'}
                </div>
              )}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 12 }}>
              <div
                style={{
                  ...
                  height: 10,
                  borderRadius: '50%',
                  background: myAnswer !== null ? 'var(--green)' : 'var(--orange)',
                  animation: 'pulse 1s ease-in-out infinite',
                }}
              />
              <span style={{ fontSize: '.76rem', color: 'var(--ink3)', fontWeight: 600 }}>
                {myAnswer !== null ? 'You answered' : 'Waiting for your answer...'}
              </span>
              {theirAnswer !== null && (
                <span
                  style={{
          ...
          >
            ...
            <div
              style={{
                ...
                fontSize: '2rem',
                fontWeight: 800,
                color: myWon ? 'var(--green)' : tied ? 'var(--yellow)' : 'var(--red)',
                marginBottom: 10,
              }}
            >
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              ...
              <a href="/trivia" className="btn btn-outline">
                Practice Solo →
              </a>
            </div>
          </div>
