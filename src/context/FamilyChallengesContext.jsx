...
export function FamilyChallengesProvider({ children }) {
  ...
  const updateChallengeProgress = async (challengeInstanceId, requirementType, increment = 1) => {
    try {
      ...
      updatedChallenge.progress.overall = calculateOverallProgress(updatedChallenge.progress.requirements)
    ...
    try {
      ...
      allParticipants.forEach(participant => {
        ...
        progress.participation[participant] = {
          ...
          rate: Math.round(((participantActive + participantCompleted) / (activeChallenges.length + completedChallenges.length)) * 100)
        }
      })
  ...
  const getChallengeLeaderboard = async () => {
    try {
      ...
      return [
        ...
        { familyName: 'Your Family', points: familyProgress.totalPoints || 0, challenges: completedChallenges.length, rank: 4 }
      ]
    } catch (error) {
  ...
  return (
    <FamilyChallengesContext.Provider value={{
      challenges,
      activeChallenges,
      completedChallenges,
      familyProgress,
