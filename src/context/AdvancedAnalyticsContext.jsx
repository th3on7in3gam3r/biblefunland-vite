...
export function AdvancedAnalyticsProvider({ children }) {
  ...
  const analyzeConsistencyPatterns = (activities) => {
    ...
    const dateRange =
      Math.floor(
        (new Date(sortedDates[sortedDates.length - 1]) - new Date(sortedDates[0])) /
          (1000 * 60 * 60 * 24)
      ) + 1;
  ...
  const generateProgressInsights = async (userId) => {
    try {
      const [activitiesResult, versesResult, badgesResult] = await Promise.all([
        ...
        requestQueue.execute(`progress-badges:${userId}`, () => db.getBadges(userId), {
          priority: 6,
          cacheable: true,
          ttl: 15 * 60 * 1000,
        }),
  ...
  const generateTimeInsights = (timePatterns) => {
    ...
    if (
      timePatterns.mostActiveDays.includes('Saturday') ||
      timePatterns.mostActiveDays.includes('Sunday')
    ) {
  ...
  const calculateLearningVelocity = (activities) => {
    ...
    const timeSpan =
      recentActivities.length > 1
        ? (new Date(recentActivities[recentActivities.length - 1].date) -
            new Date(recentActivities[0].date)) /
          (1000 * 60 * 60 * 24)
        : 1;
  ...
  const generatePatternBasedRecommendations = (patterns) => {
    ...
    if (patterns.timePatterns?.peakHours?.length > 0) {
      recommendations.push(
        `Schedule learning during your peak hours: ${patterns.timePatterns.peakHours.join(', ')}`
      );
    }
    ...
    if (patterns.activityPatterns?.mostPreferred) {
      recommendations.push(
        `Focus more on ${patterns.activityPatterns.mostPreferred} activities for better engagement`
      );
    }
  ...
  const calculateRetentionRate = (activities) => {
    ...
    const reviewed = activities.filter(
      (a) => a.activity_type === 'review' || a.activity_data?.isReview
    );
    ...
    const totalDays =
      activities.length > 1
        ? Math.max(
            1,
            Math.floor(
              (new Date(activities[activities.length - 1].completed_at) -
    ...
    try {
      ...
      return (
        result?.data?.filter((a) =>
          ['bible_reading', 'memory_verse', 'devotional'].includes(a.activity_type)
        )?.length || 0
      );
    } catch {
      return 0;
  ...
  const calculateReturnRate = (activities) => {
    ...
    const uniqueDays = new Set(
      activities.map((a) => new Date(a.completed_at || a.created_at).toDateString())
    ).size;
