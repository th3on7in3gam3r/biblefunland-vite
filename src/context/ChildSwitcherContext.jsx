...
export function ChildSwitcherProvider({ children }) {
  ...
  useEffect(() => {
    if (user?.id && profile?.role !== 'Child') {
      requestQueue
        .execute(`child-profiles:${user.id}`, () => db.getChildProfiles(user.id), {
          priority: 2,
          cacheable: true,
          ttl: 15 * 60 * 1000,
        })
        ...
        {isChildSession && activeChild ? (
          <div style={{ textAlign: 'center', padding: '20px 0' }}>
            ...
            <div style={{ display: 'flex', gap: 12 }}>
              <button onClick={onExitSession} className="btn btn-red" style={{ flex: 1 }}>
                🚪 Exit Child Session
              </button>
              <button onClick={onClose} className="btn btn-blue" style={{ flex: 1 }}>
                Continue
              </button>
            </div>
          </div>
