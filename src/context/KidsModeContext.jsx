...
function PinModal({ onConfirm, onClose, getPin }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }}>
      <div style={{ background: 'var(--surface)', borderRadius: 24, padding: '36px 32px', maxWidth: 360, width: '100%', textAlign: 'center', fontFamily: 'Poppins,sans-serif', boxShadow: '0 40px 100px rgba(0,0,0,.4)', animation: shake ? 'shake .4s ease' : 'none' }}>
        <div style={{ fontSize: '3rem', marginBottom: 12 }}>🔒</div>
        <h3 style={{ fontFamily: "'Baloo 2',cursive", fontSize: '1.4rem', fontWeight: 800, color: 'var(--ink)', marginBottom: 6 }}>Parent PIN Required</h3>
        <p style={{ fontSize: '.82rem', color: 'var(--ink3)', fontWeight: 500, marginBottom: 22, lineHeight: 1.6 }}>
          Enter your 4-digit parent PIN to turn off Kids Mode.<br />
          Default PIN is <strong>1234</strong> — change it in Settings.
        </p>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'center', marginBottom: 16 }}>
          {[0,1,2,3].map(i => (
            <div key={i} style={{ width: 48, height: 56, borderRadius: 12, border: `2.5px solid ${error ? 'var(--red)' : input.length > i ? 'var(--blue)' : 'var(--border)'}`, background: input.length > i ? 'var(--blue-bg)' : 'var(--bg2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.4rem', fontWeight: 800, color: 'var(--ink)', transition: 'all .2s' }}>
              {input.length > i ? '●' : ''}
            </div>
          ))}
        </div>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          <button onClick={onClose} style={{ flex: 1, padding: '11px 0', borderRadius: 12, border: '1.5px solid var(--border)', background: 'var(--surface)', color: 'var(--ink2)', fontFamily: 'Poppins,sans-serif', fontWeight: 700, fontSize: '.86rem', cursor: 'pointer' }}>Cancel</button>
          <button onClick={tryPin} disabled={input.length < 4} style={{ flex: 1, padding: '11px 0', borderRadius: 12, border: 'none', background: input.length === 4 ? 'var(--blue)' : 'var(--bg3)', color: input.length === 4 ? 'white' : 'var(--ink3)', fontFamily: 'Poppins,sans-serif', fontWeight: 700, fontSize: '.86rem', cursor: input.length === 4 ? 'pointer' : 'default', transition: 'all .2s' }}>Confirm</button>
        </div>
        {error && <div style={{ fontSize: '.78rem', color: 'var(--red)', fontWeight: 700, marginTop: 10 }}>❌ Wrong PIN. Try again.</div>}
      </div>
...
export const KIDS_ALLOWED_ROUTES = [
  '/', '/trivia', '/flashcards', '/map', '/game/david-goliath',
  '/game/runner', '/game/escape-room', '/game/spin-the-verse',
  '/challenge', '/activity-sheets', '/quiz/character',
  '/voice-reader', '/videos', '/prayer-beads', '/certification',
  '/prayer', '/auth', '/profile', '/dashboard', '/bible',
]
