...
export function ParentalControlsProvider({ children }) {
  ...
  return (
    <ParentalControlsContext.Provider value={{ controls, isFeatureEnabled, updateControls, isTimeUp, setIsTimeUp }}>
      {children}
...
function TimeLimitOverlay({ onExtend }) {
  return (
    <div style={{ position:'fixed', inset:0, background:'rgba(15,23,42,0.95)', zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', textAlign:'center', padding:24, backdropFilter:'blur(10px)' }}>
      <div style={{ maxWidth: 400 }}>
        <div style={{ fontSize:'5rem', marginBottom:20 }}>⏳</div>
        <h1 style={{ fontFamily:"'Baloo 2', cursive", fontSize:'2.5rem', fontWeight:800, color:'white', marginBottom:12 }}>Time's Up for Today!</h1>
        <p style={{ color:'rgba(255,255,255,0.7)', fontSize:'1.1rem', marginBottom:32, lineHeight:1.6 }}>
          You've reached the daily time limit set by your parents. Great job learning today! 🙏
        </p>
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <button onClick={() => window.location.href = '/'} style={{ padding:'16px', borderRadius:16, border:'none', background:'var(--blue)', color:'white', fontWeight:800, fontSize:'1rem', cursor:'pointer' }}>Back to Home</button>
          <p style={{ fontSize:'.8rem', color:'rgba(255,255,255,0.4)', marginTop:12 }}>Ask a parent to enter their PIN to extend time.</p>
        </div>
      </div>
    </div>
  )
