...
const DB_VERSION = 8; // Increment to force recreation/fix schema issues
  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains(CACHE_METADATA_STORE)) {
        const cacheMetadataStore = db.createObjectStore(CACHE_METADATA_STORE, {
          keyPath: 'cacheKey',
        });
      ...
      if (!db.objectStoreNames.contains(PENDING_ACTIONS_STORE)) {
        const pendingActionsStore = db.createObjectStore(PENDING_ACTIONS_STORE, {
          keyPath: 'id',
          autoIncrement: true,
        });
